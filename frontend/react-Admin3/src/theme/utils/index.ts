...
type SxInput = Record<string, any> | ((theme: Theme) => Record<string, any>) | null | undefined;

/**
 * Merges multiple sx objects into a single sx object.
 * Later objects take precedence over earlier ones for conflicting keys.
 */
export const composeSx = (...sxObjects: SxInput[]) => {
  return (theme: Theme): Record<string, any> => {
    return sxObjects.reduce<Record<string, any>>((merged, sx) => {
      if (!sx) return merged;
 ...
 */
export const productCardSx = (theme: Theme, productType?: string) => {
  const productCards = (theme.palette as any).productCards;
...
export const createGradientStyle = (mousePosition: MousePosition, isHovered: boolean, colorScheme: ColorScheme) => {
