...
export const buttonOverrides = {
  MuiButton: {
    ...
    variants: [
      ...
      {
        props: { variant: 'text' as const, size: 'small' as const },
        style: {
          "& .MuiTypography-root": {
      ...
      {
        props: { variant: 'text' as const, size: 'medium' as const },
        style: {
          "& .MuiTypography-root": {
            borderBottomWidth: '1.5px'
          },
      ...
      {
        props: { variant: 'text' as const, size: 'large' as const },
        style: {
          "& .MuiTypography-root": {
 ...
 * @param state - Visual state to force
 * @param theme - MUI theme (from useTheme or sx callback)
 * @param color - Palette color key
 * @returns sx-compatible style object
 */
...
): Record<string, any> =>
{
  ...
  const c = (p as any)[color] || p.primary;

  const focusRing = {
    outline: `1px solid ${alpha(c.main, (p.action as any).focused.ringOpacity)}`,
    outlineOffset: '2px',
  };

  const styles: Record<string, Record<string, Record<string, any>>> = {
    contained: {
      hover: {
      ...
      active: {
        ...
        '&:hover': { backgroundColor: darken(c.dark, 0.15), boxShadow: (buttonShadows.containedButton as any).activeHover },
      },
      focus: {
        ...focusRing,
    ...
    text: {
      ...
      focus: {
        ...focusRing,
        '&.Mui-focusVisible': focusRing,
        backgroundColor: alpha(c.main, p.action.activatedOpacity),
      },
    },
    outlined: {
      hover: {
