...
// src/components/User/__tests__/ValidatedPhoneInput.test.tsx
...
const renderComponent = async (props: Record<string, any> = {}) => {
  const defaultProps = {
    name: 'phone',
    onChange: vi.fn(),
...
describe('ValidatedPhoneInput', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (phoneValidationService.getCountryCodeFromName as any).mockResolvedValue('GB');
    (phoneValidationService.validatePhoneNumber as any).mockResolvedValue({
      isValid: true,
      error: null,
    });
    (phoneValidationService.formatPhoneNumber as any).mockReturnValue('020 1234 5678');
  });
