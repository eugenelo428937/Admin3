...
const SecurityStep: React.FC<StepProps> = ({
  initialData = {},
  onDataChange,
  errors = {},
  mode = "registration",
  ...
  const formRef = useRef<SecurityForm>(form);
  const onDataChangeRef = useRef(onDataChange);
  const isChangingPasswordRef = useRef(false);
  useEffect(() => { onDataChangeRef.current = onDataChange; }, [onDataChange]);

  const notifyParent = useCallback((nextForm: SecurityForm, extras: NotifyExtras = {}) => {
              ...
              <Box sx={{ textAlign: "center", mt: 2 }}>
                <Button
                  ...
                  onClick={() => {
                    ...
                    const next: SecurityForm = { ...formRef.current, password: "", confirmPassword: "" };
                    formRef.current = next;
                    setForm(next);
