...
import AdvancedFilterPanel from "./Product/AdvancedFilterPanel";
import FilterDebugger from "./Product/FilterDebugger";
import config from "../config";

// Import realistic styles
import "./RealistixProductList.css";

const RealistixProductList = React.memo(() => {
	...
	const handleClearAllFilters = useCallback(() => {
		if (process.env.NODE_ENV === "development") {
			console.log("🧹 [RealistixProductList] Clearing all filters");
		}
		// Clear panel filters
		setPanelFilters({});
	...
	const getActiveFilters = useMemo(() => {
		...
		if (subjectFilter) {
			activeFilters.push({
				...
				icon: "graduation-cap",
				color: "math", // Default color, could be mapped based on subject
			});
		}
	...
	const removeFilter = useCallback(
		(filterType, value) => {
			if (
				...
				// For URL-based filters, navigate to clear them
				const newParams = new URLSearchParams(location.search);
				if (filterType === "subject") {
					newParams.delete("subject_code");
					newParams.delete("subject");
				} else if (filterType === "group") {
					newParams.delete("group");
				} else if (filterType === "category") {
					newParams.delete("main_category");
					newParams.delete("category");
				}
				navigate(`/products?${newParams.toString()}`, { replace: true });
			} else {
				// For panel filters, update the state
	...
	// Function to fetch products with pagination (same as original)
	const fetchProducts = useCallback(
		async (page = 1, resetProducts = true) => {
			...
			if (fetchingRef.current && lastFetchParamsRef.current === paramsKey) {
				if (process.env.NODE_ENV === "development") {
					console.log(
						"🚫 [RealistixProductList] Preventing duplicate API call"
					);
				}
				return;
			}
			...
			try {
				...
				if (isSearchMode) {
					...
					if (
						...
						console.log(
							"📊 [RealistixProductList] API Filters:",
							params.toString()
						);
					}
	...
	const getProductType = (product) => {
		if (product.type === "Tutorial") return "tutorials";
		if (product.type === "Markings") return "marking";
		if (product.type === "MarkingVoucher" || product.is_voucher)
			return "voucher";
		if (
			...
			return "online";
		if (product.type === "Bundle" || product.is_bundle) return "bundle";
		if (
			product.product_name?.toLowerCase().includes("exam") ||
			product.product_name?.toLowerCase().includes("practice")
		)
			return "exam";
		return "materials";
	};
	...
	if (loading) {
		return (
			<div className="realistic-product-list">
	...
	return (
		<div className="realistic-product-list m-2">
			{/* Enhanced Header */}
					...
					<div className="active-filters-row">
						{getActiveFilters.map((filter, index) => (
							<div
								...
								className={`active-filter-chip ${filter.color}`}
								style={{ animationDelay: `${index * 100}ms` }}>
								<i className={`lucide lucide-${filter.icon}`}></i>
								<span>{filter.label}</span>
								<i
									className="lucide lucide-x remove-filter-icon"
									onClick={() =>
						...
						<button
							...
							<i className="lucide lucide-x-circle"></i>
							Clear All
						</button>
					</div>
				</div> 
			...
			{process.env.NODE_ENV === "development" && (
				<div className="filter-debugger-container">
					<button
						...
						<span className="color-light__onsurface_lkv">
							Filter Debug ({Object.keys(getActiveFilters).length}{" "}
							active)
						</span>
					</button>
					{filterDebugExpanded && (
...
{/* 
			<div className="realistic-filter-panel">
				<AdvancedFilterPanel
			...
			<div className="product-main-layout">
				...
				{!isSearchMode && (
					<aside className="realistic-filter-panel">
						<AdvancedFilterPanel
							onFiltersChange={handleFiltersChange}
							categoryFilter={categoryFilter}
							subjectFilter={subjectFilter}
				...
				{/* Main content area */}
				<main className="realistic-product-grid">
						...
						<>
							...
							<div className="products-meta">
								<div className="text-muted">
									...
									{!isSearchMode && (
										<span className="ms-2">(products & bundles)</span>
									)}
								</div>
								{hasNextPage && (
...
export default RealistixProductList;
