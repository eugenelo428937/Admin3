...
const PaymentStep = ({
      ...
      const executePaymentRules = async () => {
         ...
         try {
            ...
            const result = await rulesEngineService.executeRules(
               rulesEngineService.ENTRY_POINTS.CHECKOUT_PAYMENT,
               context
            );
            ...
            if (result.messages) {
               ...
               if (
                  setRequiredAcknowledgments &&
                  result.required_acknowledgments
               ) {
            ...
            if (result.updates) {
               ...
               if (result.updates.cart_fees) {
                  ...
                  if (bookingFee) {
                     setBookingFeeNotification(
                        `£${bookingFee.amount} tutorial booking fee has been added to your cart`
                     );
               ...
               if (result.updates.cart_fees_removed) {
                  ...
                  if (removedFee && removedFee.removed) {
                     setBookingFeeNotification(
                        "Tutorial booking fee has been removed from your cart"
                     );
                     // Refresh cart to show updated total
                     await refreshCart();
   ...
   const handleAcknowledgmentChange = async (ackKey, checked) => {
      ...
      try {
         ...
         const associatedMessage = acknowledgmentMessages.find(
            (msg) => msg.ack_key === ackKey
         );
         const messageId =
            associatedMessage?.template_id ||
            "tutorial_credit_card_acknowledgment_v1";
         ...
         const response = await rulesEngineService.acknowledgeRule(
            acknowledgmentData
         );
         ...
         if (response.success) {
            ...
            console.error(
               "💳 [PaymentStep] Failed to save acknowledgment:",
               response
            );
         }
      } catch (error) {
                  ...
                  {acknowledgmentMessages.map((message) => (
                     ...
                     >
                        ...
                        <FormControlLabel
                           ...
                           label={
                              message.content?.content?.checkbox_text ||
            ...
            {paymentMethod === "invoice" && (
               <FormControl fullWidth sx={{ mb: 3 }}>
                  ...
                  <Typography
                     ...
                     sx={{ mt: 1 }}
                  >
                     Corporate customers only. Please contact us if you don't
