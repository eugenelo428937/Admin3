/**
 * RulesEngineInlineAlert Component Tests
 */
...
describe('RulesEngineInlineAlert Component', () => {
    ...
    // Format 2: CartReviewStep message format
    const mockMessagesFormat2 = [
        {
            message_type: 'info',
            content: {
                title: 'Cart Review Message',
                message: '<p>This is a message from cart review step.</p>'
            },
            template_id: 'cart-test-1'
        },
        {
            message_type: 'warning',
            content: {
                title: 'Cart Warning',
                message: '<p>Important cart warning.</p>'
            },
            template_id: 'cart-test-2'
        }
    ];

    test('renders loading state correctly', () => {
        render(<RulesEngineInlineAlert loading={true} loadingMessage="Loading test data..." />);

        expect(screen.getByText('Loading test data...')).toBeInTheDocument();
    });

    test('renders null when no messages and not loading', () => {
        const { container } = render(<RulesEngineInlineAlert messages={[]} loading={false} />);

        expect(container.firstChild).toBeNull();
    });

    test('renders messages with titles (Format 1 - ProductList)', () => {
        render(<RulesEngineInlineAlert messages={mockMessagesFormat1} />);

        expect(screen.getByText('Test Message')).toBeInTheDocument();
        expect(screen.getByText('Warning Message')).toBeInTheDocument();
    });

    test('renders messages with titles (Format 2 - CartReviewStep)', () => {
        render(<RulesEngineInlineAlert messages={mockMessagesFormat2} />);

        expect(screen.getByText('Cart Review Message')).toBeInTheDocument();
        expect(screen.getByText('Cart Warning')).toBeInTheDocument();
    });
    ...
    test('applies correct severity variant', () => {
        const { container } = render(<RulesEngineInlineAlert messages={mockMessagesFormat1} />);
    ...
    test('renders HTML content safely', () => {
        ...
        render(<RulesEngineInlineAlert messages={messageWithHTML} />);
    ...
    test('displays correct loading icon', () => {
        const { container } = render(
            <RulesEngineInlineAlert loading={true} loadingMessage="Loading..." />
        );
    ...
    test('normalizes CartReviewStep message format correctly', () => {
        render(<RulesEngineInlineAlert messages={mockMessagesFormat2} />);

        // Should display titles from content.title
        expect(screen.getByText('Cart Review Message')).toBeInTheDocument();
        expect(screen.getByText('Cart Warning')).toBeInTheDocument();
        ...
        // Should display message from content.message
        expect(screen.getByText('This is a message from cart review step.', { exact: false })).toBeInTheDocument();
    });

    test('applies correct severity from message_type', () => {
        const { container } = render(<RulesEngineInlineAlert messages={mockMessagesFormat2} />);
