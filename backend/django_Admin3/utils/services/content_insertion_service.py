...
class EmailContentInsertionService:
    ...
    def _generate_dynamic_content(self, placeholder_name: str, rules: List[EmailTemplateContentRule], context: Dict) -> str:
        """
        Generate dynamic content for a specific placeholder.
        
        Args:
            placeholder_name (str): Name of the placeholder
            rules (List): Content rules to evaluate
            context (Dict): Email template context
            
        Returns:
            str: Generated content for the placeholder
        """
        try:
            # Get placeholder configuration
            placeholder = self._get_placeholder_config(placeholder_name)
            
            # Find matching rules for this placeholder
            matching_rules = []
            for rule_association in rules:
                rule = rule_association.content_rule
                
                # Check if this rule targets our placeholder
                if rule.placeholder_name == placeholder_name and rule.evaluate_condition(context):
                    matching_rules.append(rule_association)
                    
                    # Stop processing if this is an exclusive rule
                    if rule.is_exclusive:
                        break
            
            # Generate content from matching rules
            if matching_rules:
                contents = []
                for rule_association in matching_rules:
                    rule = rule_association.content_rule
                    
                    # Use override content if available, otherwise use rule content
                    content_template = rule_association.get_content_template()
                    variables = rule_association.get_content_variables()
                ...
                # Combine contents if multiple rules matched
                if contents:
            ...
            # Return default content if no rules matched
            if placeholder and placeholder.default_content:
                return self._render_content_template(
                    placeholder.default_content, context, {}
                )
    ...
    def _has_content_rules(self, template_name: str, placeholder_name: str) -> bool:
        ...
        try:
            return EmailTemplateContentRule.objects.filter(
                template__name=template_name,
                content_rule__placeholder_name=placeholder_name,
                content_rule__is_active=True,
                is_enabled=True
            ).exists()
        except Exception:
        ...
        try:
            ...
            for placeholder in placeholders_query:
                placeholders.append({
                    ...
                    'default_content': placeholder.default_content
                })
